<html>
<?php
$filePath="data/train.conllu";//////"data.txt";//
...
$sentI=0;
$sentIdArr=array();
for ($x = 0; $x < sizeof($sents); $x++) {
	$sent=$sents[$x];
	$toks=explode("\n", trim($sent));
	$curr=$toks[2]; //two first lines are sentence number and sentence text
	$tokParts=explode("\t",$curr); 
	if($tokParts[5]=='_'){
		$curr=$toks[2];
		$tokParts=explode("\t",$curr);
	}
...
if ($_POST) {
	...
	elseif(isset($_POST["gotoB"])){
		...
		if($key!=null){
			...
			echo '<script language="javascript">';
			echo 'alert("جمله مورد نظر یافت نشد.")';
			echo '</script>';
		}
	}
	/*echo '<pre>';
...
<div class='container'>
	...
	<form action="readConllu.php" method="post">
		...
		<input name="indx" id='senId' value="<?php echo $i; ?>" style='visibility:hidden'>
